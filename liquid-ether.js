    ...
    init() {
        ...
        class AutoDriver {
            ...
            update() {
                ...
                if (this.mouse.isHoverInside) {
                    if (this.active) this.forceStop();
                    // Reset hover state after movement? No, stay idle.
                }
                if (!this.active) {
                    this.active = true;
                ...
                super({
                    material: {
                        ...
                        uniforms: {
                            ...
                            px: { value: simProps.cellScale }, fboSize: { value: simProps.fboSize },
                            velocity: { value: simProps.src.texture }, dt: { value: simProps.dt }, isBFECC: { value: true }
                        }
                    },
                    output: simProps.dst
                });
        ...
        class ExternalForce extends ShaderPass {
            constructor(simProps) {
                ...
                const mouseM = new THREE.RawShaderMaterial({
                    ...
                    fragmentShader: externalForce_frag, blending: THREE.AdditiveBlending, depthWrite: false,
                    uniforms: { px: { value: simProps.cellScale }, force: { value: new THREE.Vector2() }, center: { value: new THREE.Vector2() }, scale: { value: new THREE.Vector2() } }
                });
            ...
            update(props) {
                ...
                uniforms.force.value.set(Mouse.diff.x * 0.5 * props.mouse_force, Mouse.diff.y * 0.5 * props.mouse_force);
        ...
        class Simulation {
            ...
            update() {
                ...
                let vel = this.options.isViscous
                    ? this.viscous.update({ viscous: this.options.viscous, iterations: this.options.iterationsViscous, dt: this.options.dt })
                    : this.fbos.vel_1;
        ...
        class WebGLManager {
            constructor(container, opts) {
                ...
                Common.init(container); Mouse.init(container); Mouse.autoIntensity = opts.autoIntensity; Mouse.takeoverDuration = opts.takeoverDuration;
                this.lastUserInteraction = performance.now();
                Mouse.onInteract = () => { this.lastUserInteraction = performance.now(); if (this.autoDriver) this.autoDriver.forceStop(); };
                this.autoDriver = new AutoDriver(Mouse, this, { enabled: opts.autoDemo, speed: opts.autoSpeed, resumeDelay: opts.autoResumeDelay, rampDuration: opts.autoRampDuration });
                this.init();
            }
            init() {
            ...
            resize() { Common.resize(); this.output.simulation.resize(); }
            loop() {
            ...
            dispose() { this.running = false; Common.renderer.dispose(); Mouse.dispose(); }
        }

        this.manager = new WebGLManager(this.container, this.options);
    }
}
